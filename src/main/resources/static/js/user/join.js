...
$(document).ready(function () {
    $("#submit").click(function (event) {
        ...
        if (
            id.length < 6 ||
            id.length > 12 ||
            !/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,12}$/.test(id)
        ) {
            var idConfirmMsg = idField.next(".confirmMsg");
            idConfirmMsg.text(
                "영어와 숫자를 조합한 6~12글자의 아이디를 입력해주세요."
            );
            idConfirmMsg.css("display", "block");
            incompleteFields.push(idField);
        } else {
            var idConfirmMsg = idField.next(".confirmMsg");
            idConfirmMsg.text("");
            idConfirmMsg.css("display", "none");

            // 아이디 중복검사
            checkDuplicate(id, function (response) {
                if (response === false) {
                    idConfirmMsg.text("중복된 아이디입니다. 다른 아이디를 선택해주세요.");
                    idConfirmMsg.css("display", "block");
                    incompleteFields.push(idField);
                } else {
                    idConfirmMsg.text("");
        ...
        if (pwField.val() !== "") {
            if (
                ...
                var pwConfirmMsg = pwField.next(".confirmMsg");
                pwConfirmMsg.text("8~15 글자의 문자/숫자/특수문자 필수 조합이어야 합니다.");
                pwConfirmMsg.css("display", "block");
                incompleteFields.push(pwField);
            } else {
                var pwConfirmMsg = pwField.next(".confirmMsg");
                pwConfirmMsg.text("");
    ...
    // 아이디 중복검사 함수
    function checkDuplicate(userId, callback) {
        $.ajax({
            url: "/users/checkDuplicate",
            type: "GET",
            data: { userId: userId },
