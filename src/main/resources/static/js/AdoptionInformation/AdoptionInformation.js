import * as reply from "../NPE단속반/reply";
//모듈 경로는 일반적으로 상대경로로 접근한다.
const boardNumber = $(".board-num").val();
let page = 1;

board.getListPage(
  { boardNumber: boardNumber, page: page },
  showBoard,
  showError
);
    ...
    text += `
        <div class="reply" data-num="${r.replyNumber}">
          ...
          <div class="reply-time">
            ${reply.timeForToday(
              r.replyRegisterDate == r.replyUpdateDate
                ? r.replyRegisterDate
                : r.replyUpdateDate
            )}
          </div>  
...
function displayAjax() {
  ...
  $.ajax({
    ...
    success: function (files) {
      ...
      files.forEach((file) => {
        let fileName =
          file.fileUploadPath + "/" + file.fileUuid + "_" + file.fileName;
        text += `
